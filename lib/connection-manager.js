...
const { ServiceDiscovery, serverKey, isLoopbackIpv4, isUsableLanIpv4, DEFAULT_TIMEOUT_MS, DEFAULT_RETRY_MS, DEFAULT_MAX_RETRY_MS } = require('./discovery')
...
class ConnectionManager extends EventEmitter {
  ...
  ensureSession(found, extra) {
    if (found.source !== 'manual' && isLoopbackIpv4(found.host || found.ip)) {
      if (this.logger) {
        this.logger.warn('Ignoring loopback CLEVER-Service target host=' +
          (found.host || found.ip) + ' port=' + found.port)
      }
      return null
    }
  ...
  handleSessionChange() {
    ...
    if (primary) {
      ...
      if (primary.source === 'discovered' && primary.registered && isUsableLanIpv4(primary.target.host)) {
        const next = {
  ...
  async start() {
    ...
    if (this.config.lastDiscovered && this.config.lastDiscovered.host) {
      if (isUsableLanIpv4(this.config.lastDiscovered.host)) {
        this.setState('searching')
        this.connectTo(Object.assign({}, this.config.lastDiscovered, { source: 'discovered' }))
      } else {
        if (this.logger) {
          this.logger.warn('Ignoring loopback lastDiscovered host=' +
            this.config.lastDiscovered.host + ' port=' + this.config.lastDiscovered.port)
        }
        this.config.lastDiscovered = null
        saveServerConfig(this.userDataDir, this.config, this.logger)
      }
    }
  ...
  startDiscovery() {
    ...
    this.discovery = new ServiceDiscovery({
      ...
      httpPorts: discoveryCfg.httpPorts,
