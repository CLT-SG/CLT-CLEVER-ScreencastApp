...
class ServiceDiscovery extends EventEmitter {
  ...
  beginRound() {
    ...
    this.setPhase('searching')
    this.sendProbe()
    this.roundTimer = setTimeout(() => this.finishRound(), this.timeoutMs)
  ...
  sendProbe() {
    ...
    const destinations = broadcastAddresses()
    this.fallbackAddresses.forEach((address) => {
      if (address && destinations.indexOf(address) === -1) {
